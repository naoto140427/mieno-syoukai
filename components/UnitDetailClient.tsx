...
export default function UnitDetailClient({ slug, initialUnit, isAdmin }: UnitDetailClientProps) {
  ...
  // Logic: DB > Mock. If both missing, 404.
  ...
  const handleSave = async () => {
    if (typeof unit.id === 'number') {
      ...
      alert('Cannot edit mock-only unit. Please add to database first.');
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-black pb-20">
      ...
      <div className="container mx-auto px-4 md:px-6 -mt-8 relative z-10">
        ...
        <motion.div
          ...
          className="bg-white dark:bg-[#0a0a0a] p-6 md:p-8 rounded-2xl shadow-xl border border-gray-100 dark:border-white/10 mb-8 relative group"
        >
          {/* Edit Button (Admin Only) */}
          {isAdmin && !isEditing && typeof unit.id === 'number' && (
            <button
              onClick={() => setIsEditing(true)}
              className="absolute top-6 right-6 p-2 text-gray-400 hover:text-mieno-navy dark:hover:text-white transition-colors opacity-0 group-hover:opacity-100"
              ...
              {isEditing ? (
                <div className="space-y-4">
                  <textarea
                    ...
                    className="w-full h-32 p-3 rounded-lg border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 text-gray-700 dark:text-gray-300 focus:ring-2 focus:ring-blue-500 outline-none resize-none"
                  />
                  <div className="flex gap-2 justify-end">
                    <button
                      onClick={handleCancel}
                      className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-sm font-medium text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
                    >
                      <X size={14} /> Cancel
                    </button>
                    <button
                      onClick={handleSave}
                      className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-sm font-medium bg-mieno-navy text-white hover:bg-opacity-90 transition-colors"
                    >
                      <Save size={14} /> Save
                    </button>
                  </div>
